...
app.get('/dashboard/summary', authRequired, async (req, res) => {
	...
	const joinProduct = await ProductModel.findAndCountAll({
		...
		products.rows.forEach(element => {
			let now = moment(new Date()), end = moment(element.valid_to), days = end.diff(now, "days")
...
app.get('/dashboard/application/:id', authRequired, async (req, res) => {
	console.log(`username ${req.user.username} at ${req.protocol}://${req.get('host')}${req.originalUrl}`)
	const productByID = await ProductModel.findByPk(parseInt(req.params.id)).then(results => {
		return results
	})
